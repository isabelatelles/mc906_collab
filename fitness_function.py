...
n_children = 200  # n children to give
n_gift_type = 40  # n types of gifts available
n_gift_quantity = 3  # each type of gifts are limited to this quantity
n_gift_pref = 100  # number of gifts a child ranks
n_child_pref = 1000  # number of children a gift ranks
twins = math.ceil(0.04 * n_children / 2.) * 2  # 4% of all population, rounded to the closest number
triplets = math.ceil(0.015 * n_children / 3.) * 3  # 1.5% of all population, rounded to the closest number
ratio_gift_happiness = 2
ratio_child_happiness = 2

gift_pref = pd.read_csv('dataset/created_child_wishlist_' + str(n_children) + '_' + str(n_gift_type) + '.csv',
                        header=None).drop(0, 1).values
child_pref = pd.read_csv('dataset/created_gift_goodkids_' + str(n_children) + '_' + str(n_gift_type) + '.csv',
                         header=None).drop(0, 1).values
...
random_sub = pd.read_csv('').values.tolist()
